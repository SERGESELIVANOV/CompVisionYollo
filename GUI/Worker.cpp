...
void Worker::setParameters(const QString& modelType, const QString& inputDir, const QString& outputDir)
{
...
void Worker::process()
{
    ...
    emit logMessage(QString("Найдено изображений для обработки: %1").arg(totalImages));
    ...
    for (const auto& entry : std::filesystem::directory_iterator(m_inputPath))
    {
        if (entry.is_regular_file() && isSupportedImage(entry.path()))
        {
            ...
            processedImages++;
            int progress = static_cast<int>((processedImages * 100) / totalImages);
            emit progressUpdated(progress);

            emit logMessage(QString("Обработано: %1/%2").arg(processedImages).arg(totalImages));
        }
    }

    emit processingFinished(true, QString("Обработано изображений: %1").arg(processedImages));
}

bool Worker::initialize()
{
    auto preset_it = m_presets.find(m_modelType.toStdString());
    if (preset_it == m_presets.end())
    {
        emit logMessage("Неизвестный preset: " + m_modelType);
        return false;
    }

    m_currentPreset = preset_it->second;
    m_inputPath = std::filesystem::path(m_inputDir.toStdString());
    m_outputPath = std::filesystem::path(m_outputDir.toStdString());
    ...
    {
        emit logMessage("Ошибка доступа к папке с изображениями: " + m_inputDir);
        return false;
    }
    ...
    {
        emit logMessage("Ошибка доступа к папке для сохранения: " + m_outputDir);
        return false;
    }
    ...
    emit logMessage("Модель успешно инициализирована: " + m_modelType);
    return true;
}

bool Worker::isSupportedImage(const std::filesystem::path& path) const
{
...
bool Worker::loadClassList(const std::string& names_path, std::vector<std::string>& class_list) const
{
